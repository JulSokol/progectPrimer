...
setInterval(function () {
    $.get('/chat/messages?gameId=' + gameId, function (result) {
        var newMessages = result.filter(m => m.id > maxMsgId);
        newMessages .forEach(m => maxMsgId = maxMsgId > m.id ? maxMsgId : m.id);
        newMessages .forEach(m => writeLine(m))
    });
}, 1000);
var emojis = JSON.parse('[{"name":null,"width":"72","height":"72"},' +
    '{"name":null,"width":"72","height":"72"},{"name":null,"width":"72","height":"72"},' +
    '{"name":null,"width":"72","height":"72"},{"name":null,"width":"72","height":"72"},' +
    '{"name":null,"width":"72","height":"72"},{"name":null,"width":"72","height":"72"},' +
    '{"name":null,"width":"72","height":"72"},{"name":null,"width":"72","height":"72"},' +
    '{"name":null,"width":"72","height":"72"},{"name":null,"width":"72","height":"72"},' +
...
function sendMessage(){
    if (input.value) {
        ...
        fetch("/chat/add", {
            method: 'POST',
            body: JSON.stringify({"message": msg, "gameId": gameId}),
            headers: {
...
function writeLine(message){
    ...
    msgDiv.innerHTML= message.user.nik + " " + message.message;
...
for(var i = 0; i < emojis.length; i++){
    ...
    emojiwrapper.innerHTML += '<img class="emoji-img" src="' + emojis[i].name + '"/>'
}
